...
#ifndef _AUDITORSYSTEMS_SPL06_h
#define _AUDITORSYSTEMS_SPL06_h
...
template <
  ...
  typename T_Address,
  typename T_Enabled,
  typename T_local_pressure,
  typename T_OutputPinsAltitude,
  typename T_OutputPinsPressure,
  ...
  public T_Address,
  public T_Enabled,
  public T_local_pressure,
  public T_OutputPinsAltitude,
  public T_OutputPinsPressure,
  ...
  public T_ClockInputPin_o_IsConnected
{
  ...
  public:
    _V_PROP_( Enabled )
  public:
    _V_PROP_( local_pressure )
  public:
    _V_PIN_( OutputPinsAltitude )
  public:
    _V_PIN_( OutputPinsPressure )
  public:
  ...
  public:
	...
	void ReadData()
      ...
      // altitude=get_altitude_f(get_pressure(),local_pressure_set); // convert from meters to feet
      float temp = get_temp_c();
      float press = get_pressure();
	  float altitude = get_altitude(get_pressure(), local_pressure_set);
      if(T_OutputPinsPressure::GetPinIsConnected())T_OutputPinsPressure::SetPinValue(press);
      if(T_OutputPinsTemperature::GetPinIsConnected())T_OutputPinsTemperature::SetPinValue(temp);
	  if(T_OutputPinsAltitude::GetPinIsConnected())T_OutputPinsAltitude::SetPinValue(altitude);
		}
    void SPL_init()
    {
    ...
    uint8_t i2c_eeprom_read_uint8_t(  uint8_t deviceaddress, uint8_t eeaddress )
    {
      uint8_t rdata = 0xFF;
      C_I2C.beginTransmission(deviceaddress);
      C_I2C.write(eeaddress);
      C_I2C.endTransmission(false); // false to not release the line

      C_I2C.requestFrom(deviceaddress, 1);
      if (C_I2C.available()) rdata = C_I2C.read();
      return rdata;
    }
  public:
